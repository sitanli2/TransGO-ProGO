...
"""使用大批量数据（如Database1时）测试时使用该工具包"""

def loading_contact_map(data_source,threshold):
    contact_map_list = os.listdir('../UltraDataset/Database1+/contact_map_dense_pdb_'+str(threshold)) #../UltraDataset/Database1_Uniprot+PDB/ or ../UltraDataset/Database1+/
    dict_contact_map = {}
    for item in tqdm(contact_map_list):
        if data_source == 'pdb':
            pid_chain_uid = item.strip().split('.')[0]
            dict_contact_map[pid_chain_uid] = np.load('../UltraDataset/Database1+/contact_map_dense_pdb_'+str(threshold)+'/'+item)
...
def loading_seq(data_source):
    dict_seq = {}
    if data_source == 'pdb':
        with open('../UltraDataset/Database1+/pdb_mapping_seq.txt','r') as f:
            for line in tqdm(f,total=7198): #精炼后的数据只有3000条了
                if '>' in line:
